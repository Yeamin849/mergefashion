                ...
                <div class="grid gap-x-8 gap-y-4 grid-cols-1">
                    @foreach ($folders as $folder)
                        <div class="border p-2">
                                ...
                                @foreach ($folder['images'] as $image)
                                    <div class="relative">
                                        <img class="w-full h-auto rounded-t-xl"
                                            src="{{ asset('images/products/' . $folder['name'] . '/' . $image) }}"
                                            alt="{{ $image }}">
                                        @can('image_delete')
                                            <form
                                        ...
                                        <button
                                            ...
                                            onclick="copyImagePath('{{ asset('images/products/' . $folder['name'] . '/' . $image) }}')">
                                            Copy
                                        </button>
                                    </div>
                                @endforeach
                            ...
                            @can('image_delete')
                                <form action="{{ route('deleteFolder', ['folderName' => $folder['name']]) }}"
                                    onsubmit="return confirm('Are you sure you want to delete this item?');"
                                    method="POST">
                                    @csrf
                                    @method('DELETE') <!-- Use this to set the HTTP method to DELETE -->
