                            ...
                            <div class="inline-flex gap-x-2">
                                ...
                                @can('write')
                                    <a class="py-2 px-3 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-blue-500 text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all text-sm dark:focus:ring-offset-gray-800"
                                        href="{{ route('admin.server_img.image_add') }}">
                ...
                <div class="grid gap-x-8 gap-y-4 grid-cols-1">
                    @foreach ($folders as $folder)
                        <div class="border p-2">
                            ...
                            <div class="grid gap-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5">
                                @foreach ($folder['images'] as $image)
                                    <div class="relative">
                                        <img class="w-full h-auto rounded-t-xl"
                                            src="{{ asset('images/server/' . $folder['name'] . '/' . $image) }}"
                                            alt="{{ $image }}">
                                        @can('image_delete', $image)
                                            <!-- Check if the user has permission to delete the image -->
                                            <form
                                                action="{{ route('server_img.deleteImage', ['folder' => $folder['name'], 'image' => $image]) }}"
                                        ...
                                        <button
                                            ...
                                            onclick="copyImagePath('{{ asset('images/server/' . $folder['name'] . '/' . $image) }}')">
                                            Copy
                                        </button>
                                    </div>
                                @endforeach
                            ...
                            @can('image_delete')
                                <form action="{{ route('server_img.deleteFolder', ['folderName' => $folder['name']]) }}"
                                    onsubmit="return confirm('Are you sure you want to delete this item?');"
                                    method="POST">
                                    @csrf
                                    @method('DELETE') <!-- Use this to set the HTTP method to DELETE -->
